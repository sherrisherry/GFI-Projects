...
in_hkrx <- function(yr, nm, cl, logf, max_try = 10){
  ...
  tmp <- tempfile()
  scripting::ecycle(save_object(object = paste('HK_trade', yr, 'rx.csv.bz2', sep = '_'), bucket = 'gfi-supplemental', file = tmp, overwrite = TRUE),
...
gfi_cty <- function(logf, max_try = 10){
  ...
  scripting::ecycle(bridge <- s3read_using(FUN = function(x)read.csv(x, colClasses=cols_bridge, header=TRUE),
