...
output <- list()
for(year in years){
  ecycle(save_object(object = paste("Comtrade",year,"input.csv.bz2",sep="-"), bucket = in_bucket, file = 'tmp/tmp.csv.bz2', overwrite = TRUE),
  ...
  output[[year]] <- m_in[, cols_out]
  rm(m_in)
}
rm(cifob_Model)
output <- do.call(rbind, output)
cat("\n","   ...writing output","\n")
file_out <- 'tmp/gaps.csv.bz2'
ecycle(write.csv(output, file = bzfile(file_out),row.names=FALSE,na=""), 
       ecycle(s3write_using(output, FUN = function(x, y)write.csv(x, file=bzfile(y), row.names = FALSE),
              ...
              logg(paste('0000', '!', paste('uploading', basename(file_out), 'failed', sep = ' '), sep = '\t')), max_try), 
       max_try,
       ecycle(put_object(file_out, basename(file_out), bucket = out_bucket), 
              logg(paste('0000', '!', paste('uploading', basename(file_out), 'failed', sep = ' '), sep = '\t')),
              max_try,
              {logg(paste('0000', '|', paste('uploaded', basename(file_out), sep = ' '), sep = '\t')); unlink(file_out)}))
